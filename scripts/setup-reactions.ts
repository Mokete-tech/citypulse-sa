 ...
 * 
 * Usage:
 * npx ts-node scripts/setup-reactions.ts
 * 
...
async function setupReactions(): Promise<void> {
  ...
  try {
    ...
    await setupReactionsAlternative();
  }
}
...
// Alternative implementation using direct database query
async function setupReactionsAlternative(): Promise<void> {
  console.log('Setting up reactions system using alternative method...');
  
  try {
    // Try using the database query endpoint
    const { data, error } = await supabase.rpc('pgexec', { sql: sqlScript });
    
    if (error) {
      throw error;
    }
    
    console.log('✅ Reactions system set up successfully using alternative method!');
    
  } catch (error: any) {
    console.error('❌ Alternative method failed. Error setting up reactions system:', error.message);
