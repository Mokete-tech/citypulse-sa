...
const EventCard: React.FC<EventProps> = ({
  ...
  // Check if the user has already ticked this event
  useEffect(() => {
    const checkLocalReaction = () => {
      const storageKey = `reaction_event_${id}`;
  ...
  const handleToggleTick = async (e: React.MouseEvent) => {
    ...
    setIsLoading(true);
    try {
      const result = await toggleReaction(id, 'event', user?.id);
  ...
  const handleShare = async (platform: 'facebook' | 'twitter' | 'whatsapp') => {
    ...
    const text = `Check out this event: ${title} at ${venue} on ${date}`;
    ...
    await recordShare(id, 'event', platform, user?.id);
